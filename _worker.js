                    ...
                    case `/${userID}`: {
                        ...
                        if ((!sub || sub == '') && (addresses.length + addressesapi.length + addressesnotls.length + addressesnotlsapi.length + addressescsv.length) == 0) {
                            if (request.headers.get('Host').includes(".workers.dev")) {
                                ...
                                subconfig = env.SUBCONFIG || "https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/config/ACL4SSR_Online_Full_MultiMode.ini";
                            }
                        }
...
async function getVLESSConfig(userID, hostName, sub, UA, RproxyIP, _url) {
    ...
    if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
        let 订阅器 = `您的订阅内容由 ${sub} 提供维护支持, 自动获取 ProxyIP: ${RproxyIP}`;
        if (!sub || sub == '') {
            if (!proxyIP || proxyIP == '') {
                订阅器 = '您的订阅内容由内置 addresses/ADD 参数提供，当前使用的 ProxyIP 为空，推荐您设置 proxyIP/PROXYIP！';
            } else {
                订阅器 = `您的订阅内容由内置 addresses/ADD 参数提供，当前使用的 ProxyIP: ${proxyIPs.join(', ')}`;
            }
        } else if (RproxyIP != 'true') {
            if (enableSocks) 订阅器 += `，当前使用的 Socks5: ${parsedSocks5Address.hostname}:${String(parsedSocks5Address.port)}`;
            else 订阅器 += `，当前使用的 ProxyIP: ${proxyIPs.join(', ')}`;
        }

        // 构建具有可变色背景和更多动画效果的 HTML 内容
        const htmlContent = `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<body>
  ...
  <div class="container">
    ...
    <p>${订阅器}</p>
    <div class="section">
      <div class="section-title">快速自适应订阅地址:</div>
      <pre id="sub1">
https://${proxyhost}${hostName}/${userID}
https://${proxyhost}${hostName}/${userID}?sub
      </pre>
      <button class="copy-btn" onclick="copyToClipboard('sub1', true)">复制</button>
    </div>
    <!-- 其他部分保持不变 -->
    <div class="section">
      <div class="section-title">Base64订阅地址:</div>
      <pre id="sub2">
https://${proxyhost}${hostName}/${userID}?b64
https://${proxyhost}${hostName}/${userID}?base64
      </pre>
      <button class="copy-btn" onclick="copyToClipboard('sub2', true)">复制</button>
    </div>
    <div class="section">
      <div class="section-title">Clash订阅地址:</div>
      <pre id="sub3">
https://${proxyhost}${hostName}/${userID}?clash
      </pre>
      <button class="copy-btn" onclick="copyToClipboard('sub3', false)">复制</button>
    </div>
    <div class="section">
      <div class="section-title">Sing-box订阅地址:</div>
      <pre id="sub4">
https://${proxyhost}${hostName}/${userID}?sb
https://${proxyhost}${hostName}/${userID}?singbox
      </pre>
      <button class="copy-btn" onclick="copyToClipboard('sub4', true)">复制</button>
    </div>
    <div class="section">
      <div class="section-title">V2Ray 配置:</div>
      <pre id="sub5">
${v2ray}
      </pre>
      <button class="copy-btn" onclick="copyToClipboard('sub5', false, true)">复制</button>
    </div>
    <div class="section">
      <div class="section-title">Clash-meta 配置:</div>
      <pre id="sub6">
${clash}
      </pre>
      <button class="copy-btn" onclick="copyToClipboard('sub6', false, true)">复制</button>
    </div>
    <div class="footer">
      <p>Telegram 交流群 技术大佬~在线发牌!</p>
      <a href="https://t.me/CMLiussss">https://t.me/CMLiussss</a>
      <p>GitHub 项目地址 Star! Star! Star!!!</p>
      <a href="https://github.com/cmliu/edgetunnel">https://github.com/cmliu/edgetunnel</a>
    </div>
  </div>
  <!-- 粒子背景动画脚本 -->
  <script>
    // 粒子背景动画
    const canvasEl = document.querySelector('#particles-js');
    ...
    // 复制功能
    function copyToClipboard(id, firstLineOnly = false, copyAll = false) {
